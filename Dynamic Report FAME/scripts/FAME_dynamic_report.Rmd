...
for (cols in colnames(ratios_rt)){
  ...
  format_list[[cols]] <- local({
    ...
    formatter("span", style = x ~ formattable::style("color" = color_list[[current_count]]))
  })
}
...
# Plot the injection time

```{r injection plot, echo=FALSE}
...
observeEvent(type_df_choices(), {
  ...
  Area_df <- tables$sample_df[,grep("Area",colnames(tables$sample_df),value = TRUE)]
  tables$Ratios_df <- (Area_df/apply(Area_df, 1,sum))*100
  tables$RT_df <- tables$sample_df[,grep("RT",colnames(tables$sample_df),value = TRUE)]
  
  
  
  # Adding necessari columns in order to plot the injection
...
observeEvent(input$choices_Type,{
  if (is.data.frame(tables$plot_df_ing)){
    if ("Type" %in% colnames(tables$plot_df_ing)){
      ...
      updateSelectInput(session, 
                      "choices_Type_batch",
                      choices = c(levels(as.factor(lubridate::day(tables$plot_df_ing$Inf_time))),"All"))
      }}
    updateSelectInput(session, 
                      "choices_Type_meta",
...
observeEvent(input$plot_it,{
  ...
  if (!input$choices_Type_batch == "" & !input$choices_Type_meta == "" & !input$choices_Type == ""){
    if (input$choices_Type == "All"){
      if (input$choices_Type_batch == "All"){
        
        plots$ING_x <- tables$plot_df_ing$Inf_time
        plots$ING_y <- tables$Ratios_df[,input$choices_Type_meta]
        
        plots$ING_snames <- tables$plot_df_ing[,"Name"]
        plots$ING_type <- tables$plot_df_ing$Type
        plots$ING_batch <- tables$plot_df_ing$Day
        
      }else{
  plots$ING_x <- tables$plot_df_ing$Inf_time[tables$plot_df_ing$Day==as.numeric(input$choices_Type_batch)]
  plots$ING_y <- tables$Ratios_df[,input$choices_Type_meta][tables$plot_df_ing$Day==as.numeric(input$choices_Type_batch)]
  ...
  plots$ING_type <- tables$plot_df_ing$Type[tables$plot_df_ing$Day==as.numeric(input$choices_Type_batch)]
      ...
      if(input$choices_Type_batch == "All"){
        plots$ING_x <- tables$plot_df_ing$Inf_time[ tables$plot_df_ing$Type == input$choices_Type]
  plots$ING_y <- tables$Ratios_df[,input$choices_Type_meta][tables$plot_df_ing$Type == input$choices_Type]
  plots$ING_snames <- tables$plot_df_ing[,"Name"][ tables$plot_df_ing$Type == input$choices_Type]
  plots$ING_batch <- tables$plot_df_ing$Day[ tables$plot_df_ing$Type == input$choices_Type]
      }else{
  ...
  plots$ING_y <- tables$Ratios_df[,input$choices_Type_meta][tables$plot_df_ing$Day==as.numeric(input$choices_Type_batch) & tables$plot_df_ing$Type == input$choices_Type]
...
renderPlotly({
  ...
  geom_smooth()
  if (input$choices_Type_batch == "All"){
    point_shape <- as.factor(plots$ING_batch)
  }else{
    point_shape <- as.factor(1)
  }
  if (input$choices_Type == "All"){
   gg <- gg+
  geom_point(aes(color = plots$ING_type,text = plots$ING_snames,shape = point_shape))+
  labs(x = "Area" , y = "Adquisition time", title = paste0("Plot of the injetion time of the ",input$choices_Type_meta),color = "Legend")
   ggplotly(gg, tooltip = "text")
    }else{
  gg <- gg+
  geom_point(aes(text = plots$ING_snames,shape = point_shape))+
  labs(x = "Area" , y = "Adquisition time", title = paste0("Plot of the injetion time of the ",input$choices_Type_meta),shape = "Legend")
...
```{r index calculations, echo = FALSE}



observeEvent(type_df_choices(), {
  if(is.data.frame(tables$sample_df)){
  df <- tables$sample_df
  Area_df <- df[,grep("Area",colnames(df),value = TRUE)]
RT_df <- df[,grep("RT",colnames(df),value = TRUE)]
  


ratios_df <- (Area_df/apply(Area_df, 1,sum))*100

ratios_df_fa <- ratios_df[,grep("FA",colnames(ratios_df),value = TRUE)]



comp_df <-as.data.frame( t(as.data.frame(lapply(str_split(gsub("\\._","",gsub("[A-Z]","",toupper(gsub("n",".",colnames(ratios_df_fa))))),"\\."),function(x){if (length(x)<3){
    x <- as.numeric(c(x,"0"))
}else{as.numeric(x)}}),col.names = colnames(ratios_df_fa))))
colnames(comp_df) <- c("Carbs","Ins","n")

SFA <- apply(ratios_df_fa[,row.names(comp_df[comp_df$Ins==0,])],1,sum)
UFA <- apply(ratios_df_fa[,row.names(comp_df[comp_df$Ins>0,])],1,sum)
MUFA <- apply(ratios_df_fa[,row.names(comp_df[comp_df$Ins==1,])],1,sum)
PUFA <- apply(ratios_df_fa[,row.names(comp_df[comp_df$Ins>1,])],1,sum)
PUFAn3 <- apply(ratios_df_fa[,row.names(comp_df[comp_df$Ins>1 & comp_df$n == 3,])],1,sum)
PUFAn6 <- apply(ratios_df_fa[,row.names(comp_df[comp_df$Ins>1 & comp_df$n == 6,])],1,sum)

ACL <- apply(ratios_df_fa,1,function(x){return(sum(as.numeric(comp_df[names(x),"Carbs"])*as.numeric(x))/100)})

DBI <- apply(ratios_df_fa[,row.names(comp_df[comp_df$Ins>0,])], 1,function(x){return(sum(x*comp_df[names(x),"Ins"]))} )

index_PI <- list(0.025,1,2,4,6,8)
PI <- apply(ratios_df_fa[,row.names(comp_df[comp_df$Ins>0,])], 1,function(x){return(sum(x*sapply(comp_df[names(x),"Ins"],function(z){index_PI[[z]]})))} )

tables$index_df <- data.frame(list(SFA,UFA,MUFA,PUFA,PUFAn3,PUFAn6,ACL,DBI,PI),row.names = df$Name)
colnames(tables$index_df) <- c("SFA","UFA","MUFA","PUFA","PUFAn3","PUFAn6","ACL","DBI","PI")
...
shorcut <- function(x){
  ...
  for (n in seq_along(x)){
    e <- x[[n]]
    if (length(grep(e[1],colnames(ratios_df_fa))) != 0&
        length(grep(e[2],colnames(ratios_df_fa))) != 0){

    ret[[(length(ret)+1)]] <- c(ratios_df_fa[,grep(e[1],colnames(ratios_df_fa),value = TRUE)]/ratios_df_fa[,grep(e[2],colnames(ratios_df_fa),value = TRUE)])
    def_cols <<- c(def_cols,vec_colanmes[[n]])
...
vec_colanmes <- c("18.0/16.0","20.0/18.0","22.0/20.0","24.0/22.0","26.0/24.0",
                  "20.1n9/18.1n9","22.1n9/20.1n9","24.1n9/22.1n9",
                  "16.1n7/16.0","18.1n7/16.1n7",
                  "18.3n6/18.2n6","20.2n6/18.2n6","20.3n6/18.3n6","20.3n6/20.2n6","20.4n6/20.3n6","20.4n6/20.3n6","22.4n6/20.4n6","24.4n6/22.4n6","22.5n6/24.5n6",
                  "18.4n3/18.3n3","20.3n3/18.3n3","20.4n3/18.4n3","20.4n3/20.3n3","20.5n3/20.4n3","22.5n3/20.5n3","24.5n3/22.5n5","24.6n3/24.5n3","22.6n3/24.6n3")
...
colnames(out_frame) <- def_cols

tables$P_S <- out_frame



}})

output$index_dt <- DT::renderDT({

  
  DT::formatRound(DT::datatable(tables$index_df),columns =c("SFA","UFA","MUFA","PUFA","PUFAn3","PUFAn6","ACL","DBI","PI"), digits = 3)  # Simple version first
})

DT::dataTableOutput("index_dt")
...
# Ratio Producte/Substrat

```{r Producte/Substrat, echo=FALSE}

output$PS_dt <- DT::renderDT({

  DT::formatRound(DT::datatable(tables$P_S ),columns = 1:ncol(tables$P_S), digits = 3)  # Simple version first
})

DT::dataTableOutput("PS_dt")
